...
Perillas: `DOWNLOAD_STRATEGY`, `DOWNLOAD_PARALLEL_WORKERS=3`, `CLIP_KEYFRAME_MARGIN_SEC=8`, `DOWNLOAD_PHASE_BUDGET_SEC=600`, `CLIP_SYNC_RETRIES=2`, `STRICT_SYNC_VALIDATION=true`, `YTDLP_CLIP_FALLBACK`, `USE_APIFY_FALLBACK` + `APIFY_TOKEN`, `WEBSHARE_PROXY_FILE|LIST|URL`, `YOUTUBE_COOKIES` (base64 o texto Netscape), `RAPIDAPI_KEY`, `USE_RAPIDAPI_DOWNLOAD`.
...
1. **Pre-corte** frame-accurate (`cut_clip`, re-encode) del rango del momento sobre la fuente resuelta → `precut.mp4`. Whisper y el render final usan **el mismo archivo** (garantiza sincronía).
2. **Whisper por clip** (`transcriber.py`): extrae audio (`extract_whisper_audio`, loudnorm, 16 kHz mono), arma un prompt de contexto (vocabulario de marca detectado en título/hook/slice del transcript + título + slice de YouTube, ≤800 chars) y llama a **Groq `whisper-large-v3-turbo`** con `verbose_json` y `timestamp_granularities=[segment, word]`; fallback OpenAI `whisper-1`. Audios >20 min van por trozos de 2 min con solape (solo OpenAI). Post-proceso: filtra alucinaciones y palabras fuera de rango, corrige "ghost words" iniciales, aplica correcciones fonéticas de marca.
3. **Refinamiento de límites** (`clip_generator.py`, `validation.py`): `snap_trim_bounds` (silencios) → `refine_bounds_to_sentences` (puntuación, gaps >0,6 s, fin de segmento, máx. 60 s) → **ancla del hook** (`find_hook_start_in_words`: si el overlay/hook/primera frase aparece entre 0,5 s y el 40 % del clip, el inicio salta ahí) → recorte de relleno inicial → ancla de primera frase. Si cambió algo, re-corta (`snapped.mp4`) y desplaza los timestamps de las palabras.
4. **Verificación** (`verify_phrases_after_snap`): compara `first_phrase_in_audio`/`last_phrase_in_audio` de la Pasada A con las palabras reales; marca `verification_failed` si fallan ambas, o si hay `incomplete_tail` (última oración cortada) o `late_hook` (recorte inicial >3 s). Métricas: `sub_coverage` (fracción del clip con subtítulos) y `words_per_sec`.
5. **Reintento de sincronía**: si `sub_coverage < 0,9` y `STRICT_SYNC_VALIDATION`, re-descarga el segmento con otro proxy (hasta 2 veces); un mismatch de frases con buena cobertura **no** re-descarga (indica análisis viejo, no descarga mala).
