...
| Worker: YouTube | `SUPADATA_API_KEY`, `RAPIDAPI_KEY`, `USE_RAPIDAPI_DOWNLOAD`, `WEBSHARE_PROXY_FILE|LIST|URL`, `YOUTUBE_COOKIES`, `DOWNLOAD_*`, `CLIP_*`, `STRICT_SYNC_VALIDATION`, `YTDLP_CLIP_FALLBACK`, `USE_APIFY_FALLBACK`, `APIFY_TOKEN`, `AUDIO_SPEED_PROBE_SEC`/`AUDIO_MIN_SPEED_KBPS`/`AUDIO_DOWNLOAD_ATTEMPTS` (W14: abandonar y reintentar con otro proxy la descarga del audio completo si está lenta), `AUDIO_MAX_DOWNLOAD_SEC` (W14-B: presupuesto del intento paciente final con el proxy más rápido si todos fueron lentos; si no entra, cae a captions), `AUDIO_RANGE_CHUNK_BYTES` (W16: tamaño del tramo Range — googlevideo estrangula a ~30 KB/s si se le pide el archivo entero de una; por tramos de 4–8 MB da >2 MB/s por el mismo proxy), `TRANSCRIPT_LANGUAGE` (W16: fija el idioma del transcript; vacío = mayoría sobre 3 tramos) | En producción: Supadata, RapidAPI y proxies |
