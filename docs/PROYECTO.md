...
- **Ranking** (`moment_selector.select_finalists`): ordena por la suma del juez (nunca por el auto-score — es la causa C4) menos penalizaciones con nombre por `verification_failed`, densidad fuera de `[1.2, 5.0]`, `bad_segment`, `payoff_not_found` e `insufficient` (fuente que no se pudo ampliar/reintentar, W1); descarta un candidato si solapa >30 % en tiempo con uno ya elegido o si su hook es casi el mismo (Jaccard de palabras sin stopwords), con backfill si la diversidad deja menos de `target`. Las notas de **todos** los candidatos (elegidos y descartados, con el motivo) se anotan en `candidates_all` dentro de `analysis_cache` (W0, sin migración de esquema).
