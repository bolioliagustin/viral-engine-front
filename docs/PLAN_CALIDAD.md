...
| **W1-C** Arrancar en el inicio de la Línea | `compute_clip_bounds(lines=...)` alinea a Línea (W4) en vez de a la palabra citada; respaldo de partículas sin Líneas | **Hecho y medido** | Sí | 20-sep (INT-3) | Hallazgo del agente de W4 (6/7 clips sin mayúscula arrancaban 1 palabra tarde). Medido en `2026-09-20-integracion-fase0.json`: `line_aligned` se activa en 93% de los clips con `whisper_full`, pero `mayúscula inicial` NO subió (45%, lejos de ≥90%) — la métrica mide la re-transcripción Whisper aislada del clip, no el transcript que usó `line_aligned` para cortar; lectura completa en `eval/runs/README.md` |
