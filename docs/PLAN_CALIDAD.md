...
| **W1-C** Arrancar en el inicio de la Línea | `compute_clip_bounds(lines=...)` alinea a Línea (W4) en vez de a la palabra citada; respaldo de partículas sin Líneas | **Hecho y medido** | Sí | 20-sep (INT-3, métrica corregida en INT-4) | Hallazgo del agente de W4 (6/7 clips sin mayúscula arrancaban 1 palabra tarde). `line_aligned` se activa en 93% de los clips con `whisper_full`. La medición de INT-3 decía "mayúscula inicial 45%" pero medía mal (re-transcripción Whisper aislada del clip, no la Línea que decidió el corte); corregido en INT-4 (`eval_metrics.resolve_capitalization`): **mayúscula inicial real (por Línea) 72,4%** — sigue bajo el objetivo de 90% pero mucho más cerca; 8 de los 27 clips `line_aligned` no matchean una Línea por ±0,3s de tolerancia justo al límite del redondeo de `start_time` (columna `integer`) — con ±0,6s subiría a 93%. Lectura completa en `eval/runs/README.md` |
